...
NER_LABELS: typing.List[str] = [
    "Person",          # For individuals like Paco Nathan, Al Gore, Tommy Lee Jones, etc.
    "Organization",    # For companies and institutions like O’Reilly, Google, Facebook, etc.
    "Publication",     # For research papers and articles like "REALM: Retrieval-Augmented Language Model Pre-Training."
    "Technology",      # For technologies like LLMs, GraphRAG, RAG, NLP, GNNs, etc.
    "Location",        # For places like Las Vegas, Harvard, etc.
    "Event",           # For conferences or events (if mentioned).
    "Research",        # For research topics or fields like entity resolution, knowledge graphs, etc.
    "University",      # For educational institutions like Harvard.
    "Country",         # For countries (if mentioned).
    "City",            # For cities like Las Vegas.
]

RE_LABELS: dict = {
    "glirel_labels": {
        "works_at": {"allowed_head": ["PERSON"], "allowed_tail": ["ORG"]},  # Person works at Organization.
        "developed_by": {"allowed_head": ["TECHNOLOGY"], "allowed_tail": ["ORG"]},  # Technology developed by Organization.
        "authored_by": {"allowed_head": ["PUBLICATION"], "allowed_tail": ["PERSON"]},  # Publication authored by Person.
        "headquartered_in": {"allowed_head": ["ORG"], "allowed_tail": ["LOC", "GPE"]},  # Organization headquartered in Location.
        "used_in": {"allowed_head": ["TECHNOLOGY"], "allowed_tail": ["RESEARCH"]},  # Technology used in Research.
        "studied_at": {"allowed_head": ["PERSON"], "allowed_tail": ["UNIVERSITY"]},  # Person studied at University.
        "no_relation": {},  # No relation between entities.
    }
}
